...
[![Version](https://img.shields.io/badge/version-0.2.0--alpha.3-2563eb.svg)](Directory.Build.props)
...
[![Tests](https://img.shields.io/badge/tests-1%2C262%20passing-2ea44f.svg)](docs/VERIFICATION.md)
...
> **Development status:** Pre-release `v0.2.0-alpha.3` — AgentNotify is under active development and has not reached the mature `v1.0.0` release. Builds are for testing and evaluation; expect incomplete features, breaking changes, and unsigned binaries.
...
Download `AgentNotifySetup.exe` from the latest tagged [GitHub prerelease](https://github.com/Akash97p/agent-notify/releases/tag/v0.2.0-alpha.3). Copy that one file to a Windows 11 machine and run it; no separate .NET runtime is required. A local packaging run creates the same ignored path at `artifacts/AgentNotifySetup.exe`.
