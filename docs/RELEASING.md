...
[`v0.2.0-alpha.2`](https://github.com/Akash97p/agent-notify/releases/tag/v0.2.0-alpha.2),
tagged 2026-09-18; the tag's release workflow publishes the Windows installer, portable archives,
checksums, and skill.
...
The workflow fails rather than publishing when tests fail, packaging fails, the version is not SemVer-style, or the tag does not exactly match the product version. Numeric assembly/file metadata is `0.2.0.0` for the current `0.2.0-alpha.2` prerelease because Windows version-resource fields are numeric; API, CLI, installer, registry, package, and release display metadata use `0.2.0-alpha.2`.
